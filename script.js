...
while (universitySeeds.length < 100) {
  const idx = universitySeeds.length + 1;
  const [country, region] = countries[idx % countries.length];
  universitySeeds.push([`Global Research University ${idx}`, country, region]);
}
